...
hosp_adm_SIMD_trend <- function(adm_simd_weekly){
  ...
  graph_data <- adm_simd_weekly %>%
    #filter(`Age Group` == input$los_age) %>%
    mutate("SIMD" = as.factor(SIMD))

  tooltip_trend <- glue("Week Ending: {graph_data$week_ending}<br>",
                        "SIMD Quintile: {graph_data$SIMD}<br>",
                        "Percent: {round(graph_data$prop, 1)}%"
  )
  ...
  graph_data %>%
    plot_ly(x = ~week_ending,
            ...
            color = ~SIMD,
            type = 'bar',
            colors = phs_graph_cols,
            text = tooltip_trend,
            hoverinfo = "text",
            marker = list(line = list(width=.5,
                                      color = 'rgb(0,0,0)'))
            # marker = list(line = list(width=.5,
            #                           color = phs_graph_cols))
            ) %>%
