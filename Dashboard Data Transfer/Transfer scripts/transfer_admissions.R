...
i_adm <- read_all_sheets(glue("{adm_path}/Archive/12_Admissions_Positives_{format(report_date -23,'%Y%m%d')}.xlsx"))
i_chiadm <- read_excel(glue("{adm_path}/Archive/CHI_Admissions_Positives_{format(report_date -23,'%Y%m%d')}.xlsx"))
...
write.csv(g_adm, glue("Test output/Admissions.csv"), row.names = FALSE)
...
write.csv(g_adm_agebd, glue("Test output/Admissions_AgeBD.csv"), row.names = FALSE)
...
write.csv(g_adm_agesex, glue("Test output/Admissions_AgeSex.csv"), row.names = FALSE)
...
write.csv(g_adm_simd, glue("Test output/Admissions_SIMD.csv"), row.names = FALSE)
