...
i_quarantine <- read_all_sheets(glue(input_data, "{format(report_date -2,'%Y%m%d')}--Border Control - Quarantine Statistics.xlsx"))
...
write.csv(g_quarantine, glue(output_folder, "Quarantine.csv"), row.names = FALSE)
